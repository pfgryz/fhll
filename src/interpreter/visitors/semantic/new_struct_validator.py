...
class NewStructValidator(IVisitor[Node]):
    ...
    def visit(self, new_struct: NewStruct) -> None:
        ...
        struct_name = self._name_visitor.name.value()
        type_name = self._name_visitor.type.take()

        if not (struct_impl := self._types_registry.get_struct(type_name)):
            raise UndefinedStructError(struct_name, new_struct.location.begin)

        for assignment in new_struct.assignments:
            self._name_visitor.visit(assignment.access)
            access = self._name_visitor.access.take()
            name = access.to_name()

            if access.is_access():
                raise InvalidFieldAssignmentError(
                    name,
                    assignment.location.begin
                )

            if name not in struct_impl.fields:
                raise AssignmentToUndefinedFieldError(
                    name,
                    struct_name,
                    assignment.location.begin
                )
